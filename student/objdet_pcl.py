...
def show_range_image(frame, lidar_name):
    ...
    range_channel_8bit = np.uint8(
        (range_channel - np.min(range_channel)) / (np.max(range_channel) - np.min(range_channel)) * 255.0
    )
    
    # step 5 : map the intensity channel onto an 8-bit scale and normalize with the difference between the 1- and 99-percentile to mitigate the influence of outliers
    intensity_channel_8bit = np.uint8(
        (intensity_channel - np.percentile(intensity_channel, 1))/ (np.percentile(intensity_channel, 99) - np.percentile(intensity_channel, 1)) * 255.0
    )
    
    # step 6 : stack the range and intensity image vertically using np.vstack and convert the result to an unsigned 8-bit integer
    stacked_image = np.vstack((range_channel_8bit, intensity_channel_8bit))
    img_range_intensity = np.uint8(stacked_image)
    
    deg90 = int(img_range_intensity.shape[1] / 4)
    img_centerr = int(img_range_intensity.shape[1]/2)
    img_range_intensity = img_range_intensity[:,img_centerr-deg90:img_centerr+deg90]
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    _, idx_height_unique = np.unique(lidar_pcl_cpy[:, :2], axis=0, return_index=True) # _means useless parameter
